...
struct server_info {
  ...
  server_info(const std::string& host,
              int                port,
...
struct inner {
  static int
  http_request(const char*, const char*, const std::string&, const std::string&, const server_info&, std::string*);
...
struct tag_caller : public builder {
  ...
  detail::field_caller& field(const std::string& k, double v, int prec = DEFAULT_PRECISION)
  {
...
{
  size_t pos = 0, start = 0;
  while ((pos = src.find_first_not_of("abcdefghijklmnopqrstuvwxyqABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.~", start)) !=
         std::string::npos) {
...
inline int inner::http_request(const char*        method,
                               const char*        uri,
...
#define _NO_MORE()                                                                                                     \
  (len >= (int)iv[0].iov_len && (iv[0].iov_len = recv(sock, &header[0], header.length(), len = 0)) == size_t(-1))
